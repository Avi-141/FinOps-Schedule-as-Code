...
from database import (
    ...
    DEFAULT_TENANT_ID, # 
    get_all_checks_for_tenant_from_db, #
    get_all_active_checks_from_db # 
)
from executor import execute_check
from cisco_chat_ai import get_cisco_ai_response 

load_dotenv()
app = FastAPI(title="FinOps Natural Language Scheduler API (Cisco AI Powered)")
...
# Try to initialize OpenAI client for fallback
try:
    client = OpenAI()
    if not os.getenv("OPENAI_API_KEY"):
        print("Warning: OPENAI_API_KEY not found - will rely on Cisco AI only")
        client = None
except Exception as e:
    print(f"Warning: Error initializing OpenAI fallback client: {e}")
    client = None
...
async def startup_event():
    print(f"🚀 Starting FinOps Scheduler with Cisco AI integration...")
    ...
    if not scheduler.running:
        ...
        print("♻️ APScheduler already running. Rescheduling jobs from DB...")
...
async def root(): 
    return {
        "message": "FinOps NL Parser Backend is running with Cisco AI!", 
        "version": "2.0.0-cisco-ai"
    }
...
async def health_check():
    ...
    # Check if Cisco AI credentials are available
    cisco_creds_available = all(os.getenv(key) for key in ["OKTA_CLIENT_ID", "OKTA_CLIENT_SECRET", "OPENAI_APPKEY"])
    openai_available = bool(os.getenv("OPENAI_API_KEY"))
    
    if cisco_creds_available:
        ai_status = "cisco_ai_available"
    elif openai_available:
        ai_status = "openai_fallback_available"
    else:
        ai_status = "no_ai_credentials"
    ...
    return {
        ...
        "cisco_ai_configured": cisco_creds_available,
        "openai_fallback_configured": openai_available
    }
...
async def parse_query_endpoint(request: QueryRequest):
    ...
    if selected_data_source_id:
        ...
        if ds_row: 
            final_data_source_id = ds_row['id']
        else: 
            print(f"⚠️ Warning: Provided dataSourceId '{selected_data_source_id}' not found for default tenant. Defaulting.")
    ...
    if not final_data_source_id: # Fallback to tenant's default CSV
        ...
        if default_source: 
            final_data_source_id = default_source['id']
            print(f"📁 Using default data source for tenant: {default_source['name']}")
        else: # Emergency fallback: first data source of the tenant
            all_tenant_sources = get_all_data_sources(DEFAULT_TENANT_ID)
            if all_tenant_sources: 
                final_data_source_id = all_tenant_sources[0]['id']
                print(f"🚨 CRITICAL: Default CSV for tenant not found. Using first available: {all_tenant_sources[0]['name']}")
            else: 
                raise HTTPException(status_code=500, detail="No data sources configured for this tenant.")
...
"""
    ...
    try:
        # Use Cisco AI with OpenAI fallback
        print("🤖 Processing query with Cisco AI...")
        completion = get_cisco_ai_response([
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"User Query: \"{natural_language_query}\""}
        ])
        
        llm_response_content = completion.choices[0].message.content
        print(f"✅ AI Raw Response: {llm_response_content}")
        ...
        if add_check_to_db(db_check_data, DEFAULT_TENANT_ID): # Pass tenant_id
            ...
            if not full_check_details_row: 
                raise HTTPException(status_code=500, detail="Failed to retrieve check after saving.")
            ...
            return {
                ...
                "suggestion": db_check_data['suggestion'], 
                "status": db_check_data['status'],
                "ai_provider": "cisco_ai"  # Indicate which AI was used
            }
        else: 
            raise HTTPException(status_code=500, detail="Failed to save check to database.")
        ...
        raise HTTPException(status_code=500, detail=f"Error processing query: {type(e).__name__} - {str(e)}")

@app.get("/api/alerts", response_model=List[dict])
async def get_alerts_api_endpoint(limit: int = 20): # Renamed
    try: 
        return get_alerts_from_db(DEFAULT_TENANT_ID, limit=limit) # Pass tenant_id
    except Exception as e: 
...
async def delete_check_api_endpoint(check_id: str): # Renamed
    ...
    try:
        ...
        if job: 
            scheduler.remove_job(check_id)
            print(f"✅ Removed job {check_id} from scheduler.")
        else: 
            print(f"⚠️ Job {check_id} not found in scheduler for removal.")
        ...
        try: 
            delete_check_from_db(check_id, DEFAULT_TENANT_ID)
        except Exception as db_e: 
            raise HTTPException(status_code=500, detail=f"DB delete error: {db_e}. Orig err: {e}")
